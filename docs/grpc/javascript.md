...
let lightning = new lnrpc.Lightning('localhost:11007', credentials);
```
...
of which is at the default `localhost:11007`, with an open channel between the
two nodes.
...
Now, create an invoice for your node at `localhost:11007`and send a payment to
...
let lnrpcDescriptor = grpc.loadPackageDefinition(packageDefinition);
let lnrpc = lnrpcDescriptor.lnrpc;
let client = new lnrpc.Lightning('some.address:11007', credentials);
